...
+ (FirebaseManager *)sharedManager
{
	...
	dispatch_once(&once, ^{
        instance = [[FirebaseManager alloc] initWithAccessToken:[NSUserDefaults standardUserDefaults].accessToken];
	});
...
- (id)initWithAccessToken:(NSString *)accessToken
{
    if (self = [super init]) {
        ...
        [self.rootFirebase authWithCredential:accessToken withCompletionBlock:^(NSError *error, id data) {} withCancelBlock:^(NSError *error) {}];
    }
